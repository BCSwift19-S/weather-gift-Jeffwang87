...
class PageVC: UIPageViewController {
    ...
    func loadLocations(){
        ...
        if let locationsArray = try? decoder.decode(Array.self, from: locationsEncoded) as [WeatherLocation] {
            ...
            print("ERROR: Couldln't decode data")
        }
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        guard let currentViewCotroller = self.viewControllers?[0] as? DetailVC else
        {return}
        locationsArray = currentViewCotroller.locationsArray
